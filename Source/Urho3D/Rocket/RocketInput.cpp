...
RocketInput::RocketInput(Context* context, ::Rocket::Core::Context* rContext): Object(context), _rocketContext(rContext)
{
	...
	SubscribeToEvent(Urho3D::E_KEYUP, URHO3D_HANDLER(RocketInput, HandleInput));
	SubscribeToEvent(Urho3D::E_KEYDOWN, URHO3D_HANDLER(RocketInput, HandleInput));
	SubscribeToEvent(Urho3D::E_MOUSEBUTTONDOWN, URHO3D_HANDLER(RocketInput, HandleInput));
	SubscribeToEvent(Urho3D::E_MOUSEBUTTONUP, URHO3D_HANDLER(RocketInput, HandleInput));
	SubscribeToEvent(Urho3D::E_MOUSEMOVE, URHO3D_HANDLER(RocketInput, HandleInput));
	SubscribeToEvent(Urho3D::E_MOUSEWHEEL, URHO3D_HANDLER(RocketInput, HandleInput));
	SubscribeToEvent(Urho3D::E_TEXTINPUT, URHO3D_HANDLER(RocketInput, HandleInput));
...
//		if ((mod & KMOD_CTRL) && eventData[KeyDown::P_KEY].GetInt() >= SDLK_a && eventData[KeyDown::P_KEY].GetInt() <= SDLK_z)
//			_rocketContext->ProcessTextInput(eventData[KeyDown::P_KEY].GetInt());
	}
		...
		_rocketContext->ProcessMouseWheel(eventData[MouseWheel::P_WHEEL].GetInt() * -1,mod);//mouse direction was flipped
